...
async def run_inference_pipeline(payload: dict):
    ...
    print(f"gula-ai: Initiating inference pipeline for study {study_id} (Patient: {patient_id})")
    ...
    for name in active_plugin_names:
        ...
        try:
            ...
            print(f"gula-ai: Ran plugin '{name}' successfully.")
        except Exception as p_err:
            print(f"gula-ai: Plugin '{name}' execution failed: {p_err}")
...
def handle_incoming_event(msg_str: str):
    try:
        ...
        if event_type == "StudyStored":
            # Run inference pipeline asynchronously
